...
from synth_pi3nn.model import make_model
...
def compute_quantiles(pi3nn_dir: PI3NNDir, mean_finn_dir: FINNDir, quantiles: np.ndarray):
    ...
    if not pi3nn_dir.is_done:
        ...
        # Data
        x = np.load(mean_finn_dir.x_train_path)
        t = np.load(mean_finn_dir.t_train_path)
        u_data = np.load(mean_finn_dir.u_train_path)[-1]

        x_eval = x.reshape(-1, 1)

        np.save(pi3nn_dir.x_path, np.squeeze(x))
        np.save(pi3nn_dir.t_path, np.squeeze(t))
        np.save(pi3nn_dir.u_data_path, np.squeeze(u_data))

        # Mean predictions
        u_pred_mean = mean_finn_dir.best_pred_u_train_test
        np.save(pi3nn_dir.u_pred_mean_path, u_pred_mean)

        # Residuals & Shift to median
        # Ensuring 50% of data is above and 50% below the center line
        raw_residuals = u_data - u_pred_mean
        median_shift = np.median(raw_residuals)
        u_pred_median = u_pred_mean + median_shift
        np.save(pi3nn_dir.u_pred_median_path, u_pred_median)
        ...
        for mask, sign, mode in [(raw_residuals < median_shift, -1, "down"), (raw_residuals > median_shift, 1, "up")]:
            x_subset = x[mask.flat].reshape(-1, 1)
            u_res_subset = (sign * (raw_residuals - median_shift))[mask.flat].reshape(-1, 1)

            model = make_model(x_subset, u_res_subset, params.n_neurons_per_layer, params.activation, positivity_method="softplus", l2_reg=params.l2_regularization)
            lr_schedule = keras.optimizers.schedules.PiecewiseConstantDecay([10**8], [params.initial_learning_rate, 1e-7])
            res_results[mode] = train_model(model, x_subset, u_res_subset, x_eval, params, lr_schedule)

            if mode == "up":
                upper_dir = pi3nn_dir.upper_dir
                res_results[mode].model.save_weights(upper_dir / "weights.weights.h5")

                np.save(upper_dir / "x_train.npy", res_results[mode].x_train)
                np.save(upper_dir / "u_res_train.npy", res_results[mode].y_train)
                np.save(upper_dir / "u_res_pred_train.npy", res_results[mode].y_train_pred)
                upper_res = res_results[mode].y_eval_pred.squeeze()
                np.save(pi3nn_dir.u_pred_upper_res_path, upper_res)

            else:
                lower_dir = pi3nn_dir.lower_dir
                res_results[mode].model.save_weights(lower_dir / "weights.weights.h5")

                np.save(lower_dir / "x_train.npy", res_results[mode].x_train)
                np.save(lower_dir / "u_res_train.npy", res_results[mode].y_train)
                np.save(lower_dir / "u_res_pred_train.npy", res_results[mode].y_train_pred)
                lower_res = res_results[mode].y_eval_pred.squeeze()
                np.save(pi3nn_dir.u_pred_lower_res_path, lower_res)

        pi3nn_dir.done_marker_path.touch()

    for q in quantiles:
        if not pi3nn_dir.get_quantile_path(q).is_file():
            u_data = np.load(pi3nn_dir.u_data_path)
            u_pred_median = np.load(pi3nn_dir.u_pred_median_path)
            upper_res = np.load(pi3nn_dir.u_pred_upper_res_path)
            lower_res = np.load(pi3nn_dir.u_pred_lower_res_path)

            # Find optimal c for both directions
            if q > 0.5:
                c = optimize_quantile("up", u_data, u_pred_median, upper_res, q)
                quantile = (u_pred_median + c * upper_res).squeeze()
            elif q < 0.5:
                c = optimize_quantile("down", u_data, u_pred_median, lower_res, q)
                quantile = (u_pred_median - c * lower_res).squeeze()
            else:
                quantile = u_pred_median.squeeze()
